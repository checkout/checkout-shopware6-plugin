document.addEventListener("DOMContentLoaded", function() {
    ...
    if (page.value === 'paymentMethodPageLoadedEvent') {
        ...
        if(form.querySelector("input[name='paymentMethodId']:checked")) {
            const radioChecked = form.querySelector("input[name='paymentMethodId']:checked").value.toString()
            isCkoPayment = radioChecked === ckoId  ? true : false
        }
        
        const invalidCardMsg = document.getElementById("cko_invalid_card").value
        const sepaErrorMessage = document.getElementById("cko_sepa_empty_iban").value
        const klarnaErrorMessage = document.getElementById("cko_klarna_method").value
        ...
        let ckoPaymentMethods = document.getElementById("cko_components_credit_card");
        ...
        if (ckoPaymentMethod) {
            if (form.querySelector("input[name='paymentMethodId']:checked").value === ckoPaymentMethodId) {
                ckoPaymentMethods.style.display = 'block'
            } else {
                ckoPaymentMethods.style.display = 'none'
            }
            Array.prototype.forEach.call(paymentMethods, function(paymentMethodRadio){
        ...
        if (document.getElementsByClassName('cko-saved-card').length > 0) {
            ...
            let saveCardRadio = saveCardRadioUl.querySelectorAll("input[name='cko-saved-card']")
        ...
        if (document.getElementsByClassName('cko-new-card').length > 0) {
            ...
            let newCardRadio = newcardRadioUl.querySelectorAll("input[name='cko-saved-card']")
        ...
        if (document.getElementsByClassName('cko-apm').length > 0) {
            ...
            let apmRadio = apmRadioUl.querySelectorAll("input[name='cko-saved-card']")
        ...
        function checkApm(event) {
            ...
            const fetchUrl = '/cko/components/store-apm-selected/'
        ...
        function validate(event) {
            ...
            if(isCkoPayment) {
                ...
                if (isSaveCard == 'new_card' || typeof(isSaveCard) === 'undefined' && typeof(apmSelected) === 'undefined' ) {
                    if(Frames.isCardValid()) {
                        ...
                        document.getElementById('cko-error-label').innerHTML = invalidCardMsg
                    ...
                    if (apmSelected === 'sepa' && document.getElementById("sepa-checkbox-input").checked == false) {
                        document.getElementById("sepa-validation").innerHTML = sepaErrorMessage
                        setTimeout(function(){
                            fadeIframeError()
                        }, 3000)
                    } else if (apmSelected === 'klarna' && document.querySelector("input[class='cko-klarna-method']:checked") === null) {
                        document.getElementById("klarna-validation").innerHTML =  klarnaErrorMessage
                        setTimeout(function(){
                            fadeIframeError()
                        }, 3000)
            ...
            if (radioSelected == 'klarna') {
                if (klarnaMethod.style.display == 'none') {
                    ...
                    if (document.getElementById("cko-apm-klarna") != null) {
                        ...
                        $("#paylater_id")[0].dataset.originalTitle = document.getElementById("klarna_paylater_message").value
                        $("#sliceit_id")[0].dataset.originalTitle = document.getElementById("klarna_sliceIt_message").value
                    }
                }
